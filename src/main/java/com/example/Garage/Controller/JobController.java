...
public class JobController {
    ...
    public ResponseEntity<List<Booking>> myAssignedJobs() {
        ...
        return ResponseEntity.ok(bookingRepo.findByAssignedMember(technician));
    }

    @PatchMapping("/{id}/accept")
    @PreAuthorize("hasAuthority('ROLE_GARAGE_MEMBER')")
    public ResponseEntity<?> acceptJob(@PathVariable Long id) {
        return ResponseEntity.ok(jobService.acceptJob(id, currentUserService.getCurrentUserId()));
    }

    @PatchMapping("/{id}/complete")
    @PreAuthorize("hasAuthority('ROLE_GARAGE_MEMBER')")
    public ResponseEntity<?> completeJob(@PathVariable Long id) {
        return ResponseEntity.ok(jobService.completeJob(id, currentUserService.getCurrentUserId()));
    }
}
